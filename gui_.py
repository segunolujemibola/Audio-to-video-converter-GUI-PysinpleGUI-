...
def save_window():
    ...
    second_layout = [
        ...
        [
            ...
            sg.In(size=(25, 1), enable_events=True, key="-FOLDER-", default_text="/converted_audio__"),
    ...
    second_window = sg.Window("Save Options", layout=second_layout, keep_on_top=True)
    while True:
...
file_list_column = [
    ...
    [
        sg.Listbox(
            values=[], enable_events=True, size=(40, 20), key="-FILE LIST-", select_mode=1
        )
    ],
    [
        sg.Button("Convert", key="-CONVERT-", disabled=True), sg.Button("Clear", key="-CLEAR LIST-", disabled=True),
    ],
...
file_preview_column = [
    ...
    [
        sg.ProgressBar(100, orientation='h', size=(20, 20), key="-PROGRESS BAR-", visible=False,
    ...
    [
        sg.Listbox(values=[], enable_events=True, size=(40, 20), key="-FILE DONE LIST-"
...
while True:
    ...
    if event == "-FOLDER-":
        if values["-BROWSE-"]:
            FILE_NAME_LIST = [file for file in converter.get_files(values["-BROWSE-"])]
            window["-FILE LIST-"].update([file.name for file in FILE_NAME_LIST])
    elif event == "-FILE LIST-" and sg.popup_yes_no("Do you want to remove this file from the queue?",
                                                    keep_on_top=True) == "Yes":
        for file in FILE_NAME_LIST:
            if file.name == values["-FILE LIST-"][0]:
                FILE_NAME_LIST.remove(file)
        window["-FILE LIST-"].update([file.name for file in FILE_NAME_LIST])
    elif event == "-CLEAR LIST-" and sg.popup_yes_no("Do you want to clear the whole queue?") == "Yes":
        FILE_NAME_LIST = []
        ...
        if proceed:
            ...
            for file in FILE_NAME_LIST:
                ...
                save_directory = converter.create_save_dir(dir_path=file_path, save_folder=directory)
                print(file_path, save_directory, sep="\n")
                file_n = converter.convert_video_to_audio_moviepy(video_file=file_path, new_dir=save_directory)
